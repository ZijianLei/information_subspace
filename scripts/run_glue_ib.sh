...
for DATASET_NAME in   rte #mrpc stsb cola #rte mrpc stsb #qnli qqp # rte mrpc stsb cola  # sst2 mnli  #cola stsb  sst2  # # cola mrpc stsb cola 
do
    ...
    do
        for lr in 8e-4 #1e-3 1e-4 1e-5 # 1e-3 1e-4 #5e-4 #1e-4 1e-5 #  5e-4 1e-4 #1e-2 1e-3 5e-3  1e-4 1e-5 5e-5  #5e-5 1e-4   #5e-5 1e-41e-5
            ...
            do #this new rate is the number of paramter is used for general information and and rest of them is used for block-wise variation
                ...
                do
                    ...
                    python -m torch.distributed.launch --nproc_per_node=$NPROC_PER_NODE \
                        ...
                        --output_dir $DIR/$TASK_NAME-result/$AdapterType/$DATASET_NAME/$DATASET_NAME-$bs-$epoch-$lr-$id-$selection_step-$direction_selection_rate-$dropout_method-$seed-lid/ \
