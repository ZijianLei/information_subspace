...
for with_ib in  True
do
    for beta in 1 1e-1  #1e-5 5e-6 1e-6 #1e-4 5e-5 1e-5 5e-6 1e-6
    do
        for DATASET_NAME in  $1  #mrpc stsb cola #rte mrpc stsb #qnli qqp # rte mrpc stsb cola  # sst2 mnli  #cola stsb  sst2  # # cola mrpc stsb cola 
        do
            ...
            do
                for lr in 1e-3 #5e-3 1e-3 5e-4 #1e-3 1e-4 1e-5 # 1e-3 1e-4 #5e-4 #1e-4 1e-5 #  5e-4 1e-4 #1e-2 1e-3 5e-3  1e-4 1e-5 5e-5  #5e-5 1e-4   #5e-5 1e-41e-5
                do  # the direction selection_rate is the dropout rate
                    ...
                    do #this new rate is the number of paramter is used for general information and and rest of them is used for block-wise variation
                        ...
                        do
                            ...
                            do
                            dropout_method=ipt #ipt
                            python -m torch.distributed.launch --nproc_per_node=$NPROC_PER_NODE run.py \
                            --model_name_or_path $DIR/roberta-base \
                                ...
                                --output_dir $DIR/$TASK_NAME-result/$AdapterType/$DATASET_NAME/$DATASET_NAME-$bs-$epoch-$lr-$id-$selection_step-$direction_selection_rate-$dropout_method-$seed-withib_$with_ib-beta-$beta/ \
                                ...
                                --beta $beta \
                                --noise_scale 0 
                                #weight decay ==  l2 norm regularization default 0
                            if [ $? -ne 0 ]; then
