...
class Trainer:
    ...
    def __init__(
        ...
        if 'ipt' not in model.dropout_method :
            self.subselector = None
    ...
    def _wrap_model(self, model, training=True):
        ...
        return model
    def add_noise(self, model, noise_scale=0):
        for n,p in model.named_parameters():
            if 'intrinsic_param' in n:
                p.data.add_( torch.randn_like(p.data),alpha = np.sqrt(noise_scale))
    def train(
        self,
        ...
        **kwargs,
        ...
        if self.subselector is not None:
            self.subselector.set_total_step(total_step=max_steps)
        ...
        fisher_tr_loss = torch.tensor(0.0).to(args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    if epoch > 0 and args.ib_beta>0:
                        energy_decay.backward(retain_graph=True)
                    for n,p in model.module.named_parameters():
                        if torch.cuda.current_device() == 1 and '5' in n:
                            print(p.grad)
                    tr_loss_step = self.training_step(model, inputs)
                    for n,p in model.module.named_parameters():
                        if torch.cuda.current_device() == 1 and '5' in n:
                ...
                if (
                    ...
                    #     print("pac_loss",energy_decay,"train_loss",tr_loss_step)                
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    # dynamic mask on grad
                    if self.subselector is not None:   
                        self.subselector.grad_drop(model,self.state.global_step)
                    ...
                    if self.deepspeed:
                        ...
                        self.optimizer.step()
                        if args.ib_beta >0:
                            self.add_noise(model.module,noise_scale = args.noise_scale)

                    if optimizer_was_run and not self.deepspeed:
                        self.lr_scheduler.step()
                     # fixed_mask after the selection step
                    ...
                    # using a large batch of data to estimate the fisher matrix
                    if self.state.global_step == 100 and self.subselector != None:
                        self.subselector.enable_grad(model)
                    # sample and update the fisher mask
                    if self.state.epoch % 1 == 0 and args.ib_beta >0 :
                        num_all_batch = 20
                        for i, inputs in enumerate(epoch_iterator):
                            if i <num_all_batch:
                                ...
                                #     # Avoid unnecessary DDP synchronization since there will be no backward pass on this example.
                                #     with model.no_sync():
                                #         fisher_tr_loss_step = self.training_step(model, inputs)
                                # else:
                                ...
                                # self.current_flos += float(self.floating_point_ops(inputs))
                                        ...
                                        # deepspeed does its own


                                model.module.store_grad()
                                model.zero_grad()
                            else:
                                break
                ...
                if self.control.should_epoch_stop or self.control.should_training_stop:
                    break
            # compute the energy decay term
            if args.ib_beta >0:
                energy_decay = model.module.compute_information_bp(num_all_batch)
                
                energy_decay = args.ib_beta * energy_decay
            # print(energy_decay)
            if step < 0:
                logger.warning(
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.do_grad_scaling:
            ...
            # print(model.forward_pre_hook.__dict__)
            # exit()
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    ...
                    if isinstance(outputs, dict):
                        ...
                        logits = tuple(v for k, v in outputs.items() if k not in ignore_keys + ["loss"]+["z"]+['hidden_attention'])
                    else:
                        logits = outputs[1:]
                else:
                    loss = None
